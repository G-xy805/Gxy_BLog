...
class WalineThemeManager {
  ...
  public async init(): Promise<void> {
    ...
    console.log('[Waline Theme Manager] Initialized with theme:', this.currentTheme);
  }
  ...
  private updateWalineTheme(): void {
    ...
    try {
      ...
      console.error('[Waline Theme Manager] Failed to update Waline theme:', error);
    }
  }
   ...
   * 获取 Waline 配置
   */
  public getWalineConfig(baseConfig: Partial<WalineThemeConfig>): WalineThemeConfig {
    const resolvedTheme = this.resolveTheme(this.currentTheme);
    const isDark = resolvedTheme === DARK_MODE;
...
export function onWalineThemeChange(listener: (theme: string) => void): () => void {
  return walineThemeManager.onThemeChange(listener);
}

export function getWalineConfig(baseConfig: Partial<WalineThemeConfig>): WalineThemeConfig {
...
if (typeof window !== 'undefined') {
  (window as any).walineThemeManager = {
    setWalineInstance: (instance: WalineInstance | null) => walineThemeManager.setWalineInstance(instance),
    ...
    onThemeChange: (listener: (theme: string) => void) => walineThemeManager.onThemeChange(listener),
