...
title: 14.策略模式
order: 14
description: 策略模式定义一系列可互相替换的算法，使算法变化独立于使用者。本文详解其定义、优缺点、结构与C++代码实现。
...
- **抽象策略（Strategy）类**：定义了一个公共接口，各种不同的算法以不同的方式实现这个接口，环境角色使用这个接口调用不同的算法，一般使用抽象基类实现；
...
定义了一个公共接口，各种不同的算法以不同的方式实现这个接口，环境角色使用这个接口调用不同的算法，一般使用抽象基类实现：
