...
```mermaid
flowchart TD
    A[主函数开始] --> B[创建cp1 - 构造函数 - 引用计数=1]
    B --> C[创建cp2 - 拷贝构造函数 - 引用计数=2]
    C --> D[创建cp3 - 默认构造函数 - 引用计数=0]
    D --> E[cp3 = cp2 - 赋值操作 - 引用计数=3]
    E --> F[cp3 = cp1 - 赋值操作 - 引用计数=3]
    F --> G[cp3 = cp3 - 自我赋值 - 无变化]
    G --> H[创建cp4 - 构造函数 - 引用计数=1]
    H --> I[cp3 = cp4 - 赋值操作 - 引用计数调整]
    I --> J[主函数结束]
    J --> K[析构函数 - 后进先出 - 释放内存]
```
...
| 操作顺序 | 操作内容 | 函数调用 | char('a')内存引用计数 | char('b')内存引用计数 | 说明 |
|---------|---------|---------|---------------------|---------------------|------|
| 1 | 创建cp1 | 构造函数 | 1 | - | 初始化为1 |
| 2 | 创建cp2 | 拷贝构造函数 | 2 | - | 拷贝增加计数 |
| 3 | 创建cp3 | 默认构造函数 | 2 | - | 空指针，计数0 |
| 4 | cp3 = cp2 | 赋值操作符 | 3 | - | cp3指向char('a')，计数增加 |
| 5 | cp3 = cp1 | 赋值操作符 | 3 | - | 同一指针，无实际变化 |
| 6 | cp3 = cp3 | 赋值操作符 | 3 | - | 自我赋值，直接返回 |
| 7 | 创建cp4 | 构造函数 | 3 | 1 | 新内存，初始化为1 |
| 8 | cp3 = cp4 | 赋值操作符 | 2 | 2 | cp3从char('a')切换到char('b') |
| 9 | 析构cp4 | 析构函数 | 2 | 1 | 计数减少 |
| 10 | 析构cp3 | 析构函数 | 2 | 0（释放） | 计数为0，释放内存 |
| 11 | 析构cp2 | 析构函数 | 1 | - | 计数减少 |
| 12 | 析构cp1 | 析构函数 | 0（释放） | - | 计数为0，释放内存 |
