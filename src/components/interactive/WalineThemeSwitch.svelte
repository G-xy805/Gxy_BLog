...
import {
	getWalineThemeStatus,
	isWalineDarkMode,
	toggleWalineTheme,
} from "@/utils/waline-theme-manager";
...
// 获取 Waline 主题状态
$: walineThemeStatus = getWalineThemeStatus();
$: walineIsDark = isWalineDarkMode();
</script>
...
>
    ...
    <!-- Waline 主题状态指示器 -->
    {#if walineThemeStatus?.isSystem}
        <div class="absolute -top-1 -right-1 w-3 h-3 bg-[var(--accent)] rounded-full border-2 border-[var(--card-bg)] animate-pulse"></div>
    {/if}
